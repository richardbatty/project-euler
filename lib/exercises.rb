...
module Euler
  ...
  def ex_8
    ...
    windows = find_windows(13, thousand_digit_number.to_s)
  ...
  def find_windows(window_length, string)
    no_of_windows = string.length - window_length
    [*0..no_of_windows].map do |window_start|
      string.slice(window_start, window_length)
    end
  end
